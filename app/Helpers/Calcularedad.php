...
class Calcularedad {
  ...
  public function armarfecha($anioxxxx, $mesxxxxx, $diaxxxxx) {
    
  }

  public function calculardnp($formulac) {
    ...
    do { //echo $anioxxxx . '-' . str_pad($mesxxxxx, 2, "0", STR_PAD_LEFT) . '-' . str_pad($diaxxxxx, 2, "0", STR_PAD_LEFT).'<br>';
      $contarxx = count(Formulacione::where('created_at', 'like', $anioxxxx . '-' . str_pad($mesxxxxx, 2, "0", STR_PAD_LEFT) . '-' . str_pad($diaxxxxx, 2, "0", STR_PAD_LEFT) . '%')->get());
      $diaxxxxx--;
      if ($contarxx > 0)
        $dianutpa++;
      if ($diaxxxxx == 0) {
